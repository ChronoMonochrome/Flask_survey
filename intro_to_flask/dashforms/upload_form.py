...
class UploadForm:
	def __init__(self, url):
		...
		self.layout = html.Div([
			...
			html.Button('Сохранить', id='save-button', style = {"display": "none"}),
			html.Button('Отменить', id='dismiss-button', style = {"display": "none"}),
			html.Button('Обновить таблицу', id='renew-button'),
			html.Div(id='student_form_container', style = {"height": "1600px"}),
			html.Div(id='dropdown_per_row_container'),
		])

		self.form = self.dashview.Add_Dash(url, layout = self.layout)

	def add_column(self, col_name, is_dropdown, foreign_model):
		self.view.columns[col_name] = MyColumn(col_name, is_dropdown, foreign_model)
...
dash_form = UploadForm("/upload_form/")
form = dash_form.form
...
def output_to_table_data(dismiss_clicks, renew_clicks):
	#if ts is None:
	#	raise PreventUpdate
